...
describe("Admin Work Item API proxy", () => {
  ...
  test("登入 User 時 GET 轉發後端 403", async () => {
    getCookie.mockReturnValue({ value: "user-jwt" });
    ...
    expect(fetch).toHaveBeenCalledWith(
      "http://localhost:5206/api/v1/admin/work-items",
      expect.objectContaining({ headers: { Authorization: "Bearer user-jwt" } }),
    );
  });

  test("登入 Admin 時 POST 轉發請求與回應", async () => {
    ...
    const response = await POST(new Request("http://localhost/api/admin/work-items", {
      method: "POST",
